...
boutton_jouer = tk.Button(fenetreJeu,text="  Jouer  ",font=("helvetica","20"),bg="green",fg="blue",command=lambda :affiche_joueur_qui_commence_fenetre(rd.choice(['yellow', 'red'])))
...
# creation de bouton sauvegarder pour sauvegarder et charger une partie 
...
def clic_souris(event):
    """
    Fonction appelée lorsqu'un clic de souris est effectué sur la grille principal.
       Cette fonction récupère les coordonnées du clic de souris et appelle la fonction placer_jeton
       pour placer un jeton dans la colonne correspondante.
    Elle prend en argument event 
    (tk.Event): Lc'est l'événement de clic contenant les coordonnées du clic

    """
    
    x, y = event.x, event.y# Récupérer les coordonnées du clic de souris
    
    
    colonne = int(x // (2 * rayon() + ESPACEMENT)) + 1# Déterminer la colonne correspondante
    
   
    placer_jeton(colonne)# Appeler placer_jeton pour placer un jeton dans cette colonne

#Fonction qui place un jeton dans la colonne spécifiée. Elle vérifie si la colonne est valide et non pleine, 
# puis appelle `affiche_grille_fenetre` pour afficher le jeton sur la grille.
def placer_jeton(colonne):
    """Place un jeton dans la colonne spécifiée.
       Cette fonction prend les coordonnées du clic de souris, détermine la colonne correspondante et 
       place un jeton dans cette colonne.
       elle prend en argument colonne."""
    # Vérifier si la colonne est valide et non pleine
    if 1 <= colonne <= NB_COLONNES:#si  la valeur de la colonne est comprise entre 1 et NB_COLONNES. indique qu'il est nécessaire de faire une d'insere la logique pour déterminer la ligne où placer le jeton dans la grille en fonction de la colonne sélectionnée." "
        ligne = ...  #si  la valeur de la colonne est comprise entre 1 et NB_COLONNES. indique qu'il est nécessaire de faire une d'insere la logique pour déterminer la ligne où placer le jeton"
        if ligne is not None:  #Cette condition vérifie si la variable ligne contient une valeur non nulle, ce qui signifie qu'une ligne valide a été déterminée pour placer le jeton.
            affiche_grille_fenetre(colonne, ligne, couleurJoueur) # La fonction affiche_grille_fenetre(colonne, ligne, couleurJoueur) prend en paramètres la colonne et la ligne où placer un jeton, ainsi que la couleur du joueur qui place le jeton.Elle affiche ensuite le jeton correspondant dans la grille du jeu à l'interface graphique en utilisant la couleur spécifiée pour le joueur.

# Lier la fonction clic_souris à l'événement de clic de souris sur le canevas principal
grille.bind("<Button-1>", clic_souris)

#Fonction qui annule le dernier coup joué en supprimant le dernier jeton placé sur la grille
def annuler_dernier_coup():
    # Récupérer tous les jetons présents sur la grille
    jetons = grille.find_withtag('jeton')#Cette ligne de code récupère tous les éléments (jetons) présents dans le canevas de la grille.
    # Vérifier s'il y a au moins un jeton sur la grille
    if jetons:
        # Sélectionner le dernier jeton placé sur la grille
        dernier_jeton = jetons[-1]
        # Supprimer le dernier jeton de la grille
        grille.delete(dernier_jeton)
    " si la liste jetons contient au moins un élément,présents sur la grille."
    "sélectionne le dernier jeton de la liste jetons avec [-1] , qui représente le dernier élément de la liste."
    "supprime le dernier jeton de la grille en utilisant la méthode delete() du canevas grille,en utilisant l'elemet du dernier jeton récupéré à la ligne précédente."
boutton_retour_en_arriere = tk.Button(fenetreJeu, text="Annuler un coup", font=("helvetica", "20"), bg="gray", fg="blue", command=annuler_dernier_coup)
boutton_retour_en_arriere.grid(row=4, column=1, padx=20, pady=20)
